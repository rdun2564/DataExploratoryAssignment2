...
# Question 2 : Have Total PM25 Emissions decreased in Baltimore City
NEI <- readRDS("summarySCC_PM25.rds")
SCC <- readRDS("Source_Classification_Code.rds")

# Step 1: Use dplyr to filter fips == "24510" ie Baltimore City
NEI <- filter(NEI, fips=="24510")

# Step 2: Examines the unbalanced nature of the data
...
with(NEIPM25Adj, plot(year,avgPM25, xlab = "", 
                      ylab ="Avg Particulate matter emitted per source(tons/yr)", ylim = c(0,11), type = "n"))
...
title (main = "Average Fine Particulates Emitted (PM25) per Source\n in Baltimore City between 1999 and 2008",
       sub = "This graph was generated after correcting for\n Number of observations per source per year") 
dev.off()
